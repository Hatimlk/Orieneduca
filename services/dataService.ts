...
const INITIAL_REGISTRATIONS: Record<string, Registration[]> = {
    '1': MOCK_REGISTRATIONS,
    '2': [],
    'user_premium': [
        { id: 101, schoolName: 'UM6P', formation: 'Science des Données', date: '01/09/2023', status: 'Admis', deadline: 'Finalisé' },
        { id: 102, schoolName: 'ENSA Agadir', formation: 'Génie Industriel', date: '15/05/2024', status: 'Liste d\'attente' }
    ]
};
...
class DataService {
    ...
    private loadData() {
        ...
        const savedRegs = localStorage.getItem('oe_regs');
        ...
        this.registrations = savedRegs ? JSON.parse(savedRegs) : INITIAL_REGISTRATIONS;
    }
    ...
    private saveData() {
        ...
        localStorage.setItem('oe_regs', JSON.stringify(this.registrations));
    }
    ...
    getRegistrations(studentId: string): Registration[] {
        return this.registrations[studentId] || [];
    }

    addRegistration(studentId: string, schoolName: string, formation: string, status: Registration['status']) {
        if (!this.registrations[studentId]) this.registrations[studentId] = [];
        this.registrations[studentId].push({
            id: Date.now(),
            schoolName,
            formation,
            status,
    ...
    deleteRegistration(studentId: string, regId: number) {
        if (!this.registrations[studentId]) return;
        this.registrations[studentId] = this.registrations[studentId].filter(r => r.id !== regId);
        this.saveData();
    }
}
