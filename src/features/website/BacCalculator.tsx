...
import { Calculator, RefreshCw, Target, TrendingUp, GraduationCap, AlertCircle, CheckCircle, ArrowRight, Sparkles, School, BookOpen, HelpCircle, ChevronRight, Trophy, Lock, Brain, MessageCircle } from 'lucide-react';
import { getSchoolThresholds, calculateLinearRegression, calculateAdmissionProbability } from '../../utils/prediction';
...
export const BacCalculator: React.FC = () => {
    ...
    const [branch, setBranch] = useState<string>('SM'); // Default to Sciences Maths
    ...
    const handleAskAI = (schoolName: string, prob: number, predicted: number) => {
        const message = `J'ai calculé une moyenne de ${average?.toFixed(2)}/20 (Branche ${branch}). Le seuil prédit pour ${schoolName} est de ${predicted.toFixed(2)} (Probabilité: ${prob}%). Que dois-je faire pour améliorer mes chances ?`;
        // Dispatch event to open chat with initial message
                        ...
                        <div className="p-6 sm:p-8">
                            <form onSubmit={calculate} className="space-y-6">
                                ...
                                <div className="bg-primary-50 p-4 rounded-xl border border-primary-100">
                                    <label className="block text-sm font-bold text-primary-900 mb-2">Votre Filière (Bac)</label>
                                    <div className="grid grid-cols-2 gap-2">
                                        {['SM', 'PC', 'SVT', 'Eco', 'Tech', 'Lettres'].map((b) => (
                                            <button
                                                ...
                                                type="button"
                                                onClick={() => setBranch(b)}
                                                className={`py-2 px-3 rounded-lg text-sm font-semibold transition-all ${branch === b
                                ...
                                <div className="flex gap-3 pt-2">
                                    ...
                                    <button
                                        type="submit"
                                        className={`flex-1 text-white px-6 py-3.5 rounded-xl font-bold text-lg shadow-lg transition-all transform hover:-translate-y-0.5 hover:shadow-xl flex items-center justify-center ${mode === 'AVERAGE'
                    ...
                    {!showResults ? (
                        ...
                        <div className="space-y-6 animate-fade-in-up">
                            ...
                            <div className="bg-white rounded-[2rem] shadow-xl shadow-gray-200/50 border border-gray-100 p-8 md:p-10 relative overflow-hidden">
                                ...
                                <div className={`absolute top-0 right-0 w-[400px] h-[400px] rounded-full -mr-40 -mt-40 blur-[100px] opacity-30 ${(average || 0) >= 16 ? 'bg-emerald-400' : (average || 0) >= 12 ? 'bg-blue-400' : 'bg-orange-400'
                                    }`}></div>
                                ...
                                <div className="bg-white rounded-[2rem] shadow-lg border border-gray-100 p-8">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4">
                                        {getOpportunities(mode === 'AVERAGE' ? average! : parseFloat(target)).map((opp, idx) => {
                                            ...
                                            // Smart Seuil Prediction
                                            let predictedThreshold = opp.min;
                                            let probability = 0;
                                            let isSmartPrediction = false;

                                            const historical = getSchoolThresholds(opp.name, branch);
                                            if (historical) {
                                                const years = Object.keys(historical.years).map(Number);
                                                const thresholds = Object.values(historical.years);
                                                predictedThreshold = calculateLinearRegression(years, thresholds);
                                                probability = calculateAdmissionProbability(score, predictedThreshold);
                                                isSmartPrediction = true;
                                            } else {
                                                // Fallback logic
                                                probability = score >= opp.min ? 90 : score >= opp.min - 1 ? 40 : 10;
                                            }

                                            const isSafe = probability > 70;
                                            const isRisk = probability < 40;

                                            return (
                                                <div key={idx} className="flex flex-col p-4 rounded-2xl border border-gray-100 bg-white hover:border-primary-200 hover:shadow-md transition-all group relative overflow-hidden">
                                                    {isSafe && <div className="absolute left-0 top-0 bottom-0 w-1 bg-green-500"></div>}
                                                    {isRisk && <div className="absolute left-0 top-0 bottom-0 w-1 bg-orange-500"></div>}
                                                    {!isSafe && !isRisk && <div className="absolute left-0 top-0 bottom-0 w-1 bg-yellow-500"></div>}

                                                    <div className="flex items-center mb-3">
                                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center mr-3 shrink-0 ${isSafe ? 'bg-green-50 text-green-600' : isRisk ? 'bg-orange-50 text-orange-600' : 'bg-yellow-50 text-yellow-600'}`}>
                                                            {opp.icon}
                                                        </div>
                                                        <div className="min-w-0">
                                                            <h4 className="font-bold text-gray-900 truncate">{opp.name}</h4>
                                                            <span className="text-[10px] uppercase tracking-wide text-gray-400 font-bold">{opp.category}</span>
                                                        </div>
                                                    </div>

                                                    <div className="space-y-2">
                                                        <div className="flex justify-between items-center text-sm">
                                                            <span className="text-gray-500">Seuil Prédit 2026</span>
                                                            <span className="font-mono font-bold text-gray-900">{isSmartPrediction ? predictedThreshold.toFixed(2) : `~${opp.min}`}</span>
                                                        </div>

                                                        <div className="space-y-1">
                                                            <div className="flex justify-between text-xs font-bold">
                                                                <span className={isSafe ? 'text-green-600' : isRisk ? 'text-orange-600' : 'text-yellow-600'}>
                                                                    {isSafe ? 'Admission Probable' : isRisk ? 'Difficile' : 'Possible'}
                                                                </span>
                                                                <span>{isSmartPrediction ? `${probability.toFixed(0)}%` : '-'}</span>
                                                            </div>
                                                            <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden">
                                                                <div
                                                                    className={`h-full rounded-full transition-all duration-1000 ${isSafe ? 'bg-green-500' : isRisk ? 'bg-orange-500' : 'bg-yellow-500'}`}
                                                                    style={{ width: `${isSmartPrediction ? probability : isSafe ? 90 : 40}%` }}
                                                                ></div>
                                                            </div>
                                                        </div>

                                                        {!isSafe && isSmartPrediction && (
                                                            <button
                                                                onClick={() => handleAskAI(opp.name, probability, predictedThreshold)}
                                                                className="w-full mt-2 py-2 flex items-center justify-center text-xs font-bold text-primary-700 bg-primary-50 rounded-lg hover:bg-primary-100 transition-colors"
                                                            >
                                                                <Brain className="w-3 h-3 mr-1.5" /> Comment améliorer mes chances ?
                                                            </button>
                                                        )}
                                                    </div>
                                                </div>
