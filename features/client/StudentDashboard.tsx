...
import { User, NavPage, StudentTask, StudentDocument, Registration, RegistrationStatus } from '../../types';
import {
    Calendar, Globe, FileText, CheckSquare, Clock, Plus, Trash2, Calculator, Trophy, Target, Sparkles, Flame, FolderOpen, Upload, Award, CheckCircle, BookOpen, Layout, Zap, X, User as UserIcon, Mail, Phone, MapPin, Building, GraduationCap, Download, PenTool, ExternalLink
} from 'lucide-react';
...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({ user: initialUser, onNavigate }) => {
    ...
    const [activeTab, setActiveTab] = useState<'DASHBOARD' | 'PROFILE' | 'REGISTRATIONS'>('DASHBOARD');

    // Registration State
    const [registrations, setRegistrations] = useState<Registration[]>([]);
    const [isAddingReg, setIsAddingReg] = useState(false);
    const [newReg, setNewReg] = useState({ schoolName: '', formation: '', status: 'En attente' as RegistrationStatus });
    ...
    const getStatusColor = (status: RegistrationStatus) => {
        switch (status) {
            case 'Admis': return 'bg-green-100 text-green-700 border-green-200';
            case 'Admissible': return 'bg-blue-100 text-blue-700 border-blue-200';
            case 'Refusé': return 'bg-red-100 text-red-700 border-red-200';
            case 'Liste d\'attente': return 'bg-orange-100 text-orange-700 border-orange-200';
        ...
        <div className="animate-fade-in">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-900 flex items-center">
                    <PenTool className="w-6 h-6 mr-3 text-primary-600" /> Suivi de mes Inscriptions
                </h2>
                <button
                    onClick={() => setIsAddingReg(true)}
                    className="bg-primary-600 text-white px-4 py-2 rounded-xl text-sm font-bold flex items-center hover:bg-primary-700 transition-colors shadow-sm"
                >
            ...
            {isAddingReg && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl p-6 w-full max-w-md animate-fade-in shadow-2xl">
                        <h3 className="text-xl font-bold text-gray-900 mb-4">Ajouter une inscription</h3>
                        <form onSubmit={handleAddRegistration} className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Établissement</label>
                                <input
                                    type="text"
                                    value={newReg.schoolName}
                                    onChange={(e) => setNewReg({ ...newReg, schoolName: e.target.value })}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Formation / Filière</label>
                                <input
                                    type="text"
                                    value={newReg.formation}
                                    onChange={(e) => setNewReg({ ...newReg, formation: e.target.value })}
                                    className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary-500 outline-none"
                            ...
                            <div>
                                ...
                                <select
                                    value={newReg.status}
                                    onChange={(e) => setNewReg({ ...newReg, status: e.target.value as RegistrationStatus })}
                                    className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary-500 outline-none"
                                >
                                    <option value="En attente">En attente</option>
                                    <option value="Admissible">Admissible (Pré-sélectionné)</option>
            ...
            <div className="grid grid-cols-1 gap-4">
                {registrations.length === 0 ? (
                    <div className="text-center py-12 bg-white rounded-3xl border border-gray-200 border-dashed">
                        ...
                        <h3 className="text-lg font-bold text-gray-900 mb-2">Aucune inscription suivie</h3>
                        <p className="text-gray-500 mb-6">Ajoutez les écoles auxquelles vous avez postulé pour suivre leur état.</p>
                        <button
                            onClick={() => setIsAddingReg(true)}
                    ...
                    registrations.map((reg) => (
                        <div key={reg.id} className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-all group relative">
                            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                                <div className="flex items-start">
                                    <div className="w-12 h-12 rounded-xl bg-gray-100 flex items-center justify-center mr-4 shrink-0 font-bold text-xl text-gray-500">
                                        {reg.schoolName.charAt(0)}
                                    </div>
                                    <div>
                                        <h3 className="font-bold text-lg text-gray-900">{reg.schoolName}</h3>
                                        <p className="text-gray-600 font-medium">{reg.formation}</p>
                                        <div className="flex items-center mt-2 text-xs text-gray-400 space-x-3">
                                            <span className="flex items-center"><Calendar className="w-3 h-3 mr-1" /> Ajouté le {reg.date}</span>
                                            {reg.deadline && <span className="flex items-center text-orange-500"><Clock className="w-3 h-3 mr-1" /> Deadline: {reg.deadline}</span>}
                                        </div>
                                    </div>
                                </div>

                                <div className="flex items-center gap-4 self-end md:self-auto">
                                    <span className={`px-4 py-1.5 rounded-full text-sm font-bold border ${getStatusColor(reg.status)}`}>
                                        {reg.status}
                                    </span>
                                    <button
                            ...
                            {reg.notes && (
                                <div className="mt-4 pt-4 border-t border-gray-100 flex items-start text-sm text-gray-600 bg-gray-50/50 p-3 rounded-lg">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
            ...
            <div className="flex space-x-1 bg-gray-200/50 p-1 rounded-xl mb-8 w-fit">
                ...
                <button
                    ...
                    className={`px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'REGISTRATIONS' ? 'bg-white text-primary-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    Mes Inscriptions
                </button>
            </div>
                                    ...
                                    {documents.map((doc) => (
                                        <div key={doc.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100 group relative">
                                            ...
                                            <div className="flex items-center ml-2 shrink-0 gap-1">
                                                <span
                                                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full mr-1 ${doc.status === 'Validé'
