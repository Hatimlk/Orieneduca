...
export const AICounselor: React.FC<AICounselorProps> = ({ isOpen, onToggle }) => {
    ...
    const handleOpenChat = (event: Event) => {
      ...
      if (customEvent.detail && customEvent.detail.message) {
        ...
        setTimeout(() => {
          ...
          // Optionally auto-send: handleSendMessage with the message
        }, 100);
      }
    };
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[100] flex flex-col items-end">
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`flex max-w-[90%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`flex-shrink-0 h-8 w-8 rounded-xl flex items-center justify-center mt-1 shadow-sm ${msg.role === 'user' ? 'bg-primary-600 text-white ml-2' : 'bg-white border border-gray-200 text-primary-600 mr-2'
                    }`}>
                    {msg.role === 'user' ? <User className="h-4 w-4" /> : <Sparkles className="h-4 w-4" />}
                  </div>

                  <div className="space-y-2">
                    <div className={`px-4 py-3 rounded-2xl shadow-sm text-sm leading-relaxed ${msg.role === 'user'
      ...
      <button
        onClick={onToggle}
        className={`p-4 rounded-2xl shadow-2xl transition-all duration-500 transform hover:scale-110 flex items-center justify-center group ${isOpen
